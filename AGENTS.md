...
- **[llms.txt](./llms.txt)** — Package map: every export grouped by import path, one line each
- **[llms-full.txt](./llms-full.txt)** — Full reference: props tables, signatures, examples, and anti-patterns

Both files must be kept up to date. When adding a new exported component, hook, or utility — or changing an existing one, update both files to reflect the change.
